...
const EditSocialMedias: React.FC = () => {
    ...
    const { socialmedia } = location.state || {};
    const [id] = useState<number>(socialmedia.id);
    const [type, setType] = useState<string>(socialmedia.type);
    const [label, setLabel] = useState<string>(socialmedia.label);
    const [url, setUrl] = useState<string>(socialmedia.url);
    ...
    const handleUpate = async (event: React.FormEvent) => {
        ...
        if (!isValidation1) {
            ...
            try {
                await fetchUpdate('social-medias', id, apiServiceJson, token);

                showSnackbar('Red social actualizada correctamente.', 'success');
